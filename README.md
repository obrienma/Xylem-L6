...
> [!NOTE]
> **Status: Phase 6 complete, not yet wired to Sentinel-L7.** All four trackers — Phase 1's sliding-window velocity counter and Phase 2's first-seen IP tracker, impossible travel detector, and scope escalation tracker — checkpoint their combined state to a local JSON file after every event and restore it on startup (Phase 3). [ADR 0004](docs/adr/0004-sentinel-l7-sink-decision.md) fixes Xylem-L6's Phase 4 sink as **Sentinel-L7** — a decision, not an integration. [ADR 0005](docs/adr/0005-fusion-function-max-of-signals.md) fixes the fusion function as max-of-signals — now **implemented** as a pure function (`src/core/fusion.ts`), with no consumer yet. [ADR 0006](docs/adr/0006-tenant-label-on-api-activity-event.md) adds an optional `tenant` field to `ApiActivityEvent`, prompted by a customer-attribution gap in Ledger-L5's billing contract — implemented, including a `fixture-replay` scenario demonstrating (not fixing) the single-tenant tracker-keying limitation. Sentinel-L7's SaaS-domain policy corpus is Accepted on its side (Sentinel-L7 ADR-0032) but still blocked there on a producer-side `domain` stamp; no MCP client or Axiom-shaped output exists in Xylem-L6 yet, so nothing calls Sentinel-L7 today.
...
- **Fusion function (`src/core/fusion.ts`, Phase 4 / [ADR 0005](docs/adr/0005-fusion-function-max-of-signals.md)):** pure function, no tracker state of its own. Normalizes each of the four signals independently to a severity in `[0, 1]` — boolean signals (first-seen IP, scope escalation) fire at `1.0`; continuous signals (velocity, impossible-travel speed) scale by the same thresholds `index.ts` uses for its console bracket flags (`src/core/thresholds.ts`, the single source of truth for both) — and returns the max of the four, plus which signal(s) produced it. Not yet called from anywhere; no consumer exists until Sentinel-L7 wiring does.

**🧪 Testing & Dev**

- **Vitest:** Test runner — `tests/` mirrors `src/`, colocated by module. 55 tests covering the schema, the velocity counter (including the late/out-of-order case), the three Phase 2 signal trackers, the Phase 3 checkpoint store, state round-trips for all four trackers, the fusion function (dataset-driven over signal combinations, boundary/tie cases), the ADR 0006 tenant collision scenario, both adapters (GitHub calls mocked via an injectable `fetchImpl`, never a real network call), and a domain-isolation arch test (`tests/arch.test.ts`).
...
* **Sentinel-L7 scored-output integration** — the sink is decided ([ADR 0004](docs/adr/0004-sentinel-l7-sink-decision.md)) and the fusion function is implemented (`src/core/fusion.ts`, [ADR 0005](docs/adr/0005-fusion-function-max-of-signals.md)), but nothing calls it and nothing calls Sentinel-L7. Sentinel-L7's SaaS-domain policy corpus is now Accepted on its side (Sentinel-L7 ADR-0032) but self-described as blocked on `WatchAxioms`/the Synapse-L4 emitter stamping `domain: 'saas'` on real Axiom payloads. Per [ADR 0002](docs/adr/0002-sentinel-l7-integration-direction.md), the intended shape is a typed Axiom contract over Sentinel-L7's MCP endpoint — the same pattern Synapse-L4 uses — so that producer-side domain-stamping gap is likely something Xylem-L6's own wiring work has to close, not a separate, independently-resolving concern. No MCP client or Axiom-emitting code exists in Xylem-L6 yet.
