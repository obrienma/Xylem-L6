...
| [ADR 0006](docs/adr/0006-tenant-label-on-api-activity-event.md) | Optional `tenant` field on `ApiActivityEvent`, prompted by Ledger-L5's customer-attribution gap; Proposed, not yet implemented | 2026-07-16 |
...
* **Optional `tenant` field on `ApiActivityEvent`** — decided ([ADR 0006](docs/adr/0006-tenant-label-on-api-activity-event.md)), not yet added to `src/core/types.ts`. Tracker keying stays `actor.id`-only (no composite key) until a second real tenant exists. The `fixture-replay` schedule also needs revising to interleave at least two synthetic tenants (e.g. `acme-corp`, `globex`) so the field has an exercised path and the single-tenant tracker-keying limitation is demonstrable.
